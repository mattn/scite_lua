//build@ gcc win32-spawner-ex.c  -I..\lua\5.3\src libscite.a 
...
static int
SendToHandle(HWND hwnd, const char *Message) {
  COPYDATASTRUCT cds;
...
static int
do_foreground(lua_State* L) {
  SetForegroundWindow(hSciTE);
  return 0;
}

static int
do_verbose(lua_State* L) {
  return 0;
}

static int
do_fulllines(lua_State* L) {
...
static void
exec_lua(lua_State* L, const char* fun, const char* args) {
  lua_getglobal(L,fun);
...
static void
send_to_scite(lua_State* L, const char* fun, const char* args) {
...
static int
spawner_set_output(lua_State* L) {
...
static int
spawner_set_result(lua_State* L) {
  Spawner* spp = (Spawner*)lua_touserdata(L,1);
...
spawner_run(lua_State* L) {
  ...
  if (spp->output != NULL) {
	...
	int fd = _open_osfhandle((intptr_t)spp->hPipeRead, _O_BINARY);
...
static int
do_popen2(lua_State* L) {
  Spawner* spp = start_popen(L);
  newfile_from_handle(L,spp->hWriteSubProcess,"wb");
  newfile_from_handle(L,spp->hPipeRead,"rb");
  return 2;
}

static int
spawner_kill(lua_State* L) {
  Spawner* spp = (Spawner*)lua_touserdata(L,1);
...
/* based on liolib.c */
static void
newfile_from_handle(lua_State *L, HANDLE handle, const char* mode) {
...
static const  luaL_Reg spawner_methods[] = {
...
luaopen_spawner(lua_State *L) {
  hSciTE = GetForegroundWindow();
  hInstance =(HINSTANCE) (intptr_t)GetWindowLong(hSciTE, GWLP_HINSTANCE);
